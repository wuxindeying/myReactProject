...
export default class RichText extends Component {
  ...
  state = {
    editorState: EditorState.createEmpty()//初始化一个空的编辑器状态
	}

	//根据富文本还原出效果文本，以及编辑器状态
	setRichText = (html)=>{
    const contentBlock = htmlToDraft(html);//将html转换成编辑器需要的格式
    if (contentBlock) {
      const contentState = ContentState.createFromBlockArray(contentBlock.contentBlocks);
      const editorState = EditorState.createWithContent(contentState);//根据内容创建相应的编辑器状态
      this.setState({editorState})//放进去
    }
	}
